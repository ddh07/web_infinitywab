<aside class="w-64 bg-surface-raised border-r border-(--border-default) h-screen">
    <div class="flex h-full flex-col">
        <div class="px-4 py-6 border-b border-(--border-default)">
            <div class="text-lg font-bold text-ink-primary">Infinity WAB</div>
            <p class="text-xs text-ink-muted mt-0.5">Administration</p>
        </div>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            @php
                $items = [
                    ...
                    ['route' => 'admin.media', 'label' => 'Fichiers', 'icon' => 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14M14 8h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'],
                    ...
                    ['route' => 'admin.faqs', 'label' => 'FAQ', 'icon' => 'M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['route' => 'admin.testimonials', 'label' => 'Avis clients', 'icon' => 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z'],
                    ['route' => 'admin.team', 'label' => 'Équipe', 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z'],
            ...
            <a href="{{ route($item['route']) }}" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs($item['route'].'*') ? 'bg-azure-50 text-azure-700 dark:bg-azure-500/10 dark:text-azure-300' : 'text-ink-secondary hover:bg-black/5 hover:text-ink-primary dark:hover:bg-white/5' }}">
        ...
        <div class="border-t border-(--border-default) p-3">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-ink-secondary hover:bg-red-50 hover:text-red-600 transition">
                    <svg class="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
